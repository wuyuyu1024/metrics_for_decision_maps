...
class StochasticEmbedding(BaseEstimator):
    ...
    def _fit(self, X, Y, lab=None, direct_adaption=True, eta=0.1, max_itr=500, F=None):
        ...
        if lab is None:
            # print('@@@@ n_centroids', n_centroids)
            centroieds = KMeans(n_clusters=n_centroids, random_state=42).fit(X).cluster_centers_
            # print('@@@@ centroieds', centroieds.shape)
        else:
            n = lab.max()+1
        ...
        # ax.triplot(self._centroieds[:,0], self._centroieds[:,1], self._triangolation.simplices, linewidth=0.1)
        # ax.scatter(X[:,0], X[:,1], marker='s', alpha=0.3, facecolors='none', edgecolors='r')
        # ax.scatter(self._centroieds[:,0], self._centroieds[:,1], marker='.', )
        
        # # ax.set_xlim(np.min(X, axis=0)[0]-0.1, np.max(X, axis=0)[0]+0.1)
        # # ax.set_ylim(np.min(X, axis=0)[1]-0.1, np.max(X, axis=0)[1]+0.1)
        # plt.show()
        ##############################
